...
|------|------|--------|------|
| 重力加速度 | 0.9 | 1.4 | 2.2 |
| 跳跃力（瞬时上升速度） | -0.50 | -0.55 | -0.58 |
| 最大速度 maxVelocity | 1.0 | 1.2 | 1.5 |
| 管道水平间距 | 0.55 | 0.45 | 0.38 |
| 管道缺口大小 | 0.28 | 0.22 | 0.20 |
| 管道滚动速度 | 0.25 | 0.35 | 0.45 |
| 地面滚动速度 | 同管道速度 | 同管道速度 | 同管道速度 |
| 缺口位置范围（Y） | 0.2 ~ 0.7 | 0.15 ~ 0.75 | 0.12 ~ 0.74 |
...
- 首根管道在 `x = worldWidth + 0.4` 处生成（给新手额外 1.2s 缓冲时间）
...
- **Ready：** 小鸟在 `(bird.x, 0.35)` 微微上下浮动（振幅 0.02，频率 3 rad/s），显示 "Tap to Start"（白色，字号 0.04 × canvasHeight，居中），管道不动，云朵和地面不滚动
- **Playing：** 第一次点击后进入，开始物理和滚动
- **Dead：** 碰撞后进入，保留当前速度（`bird.velocity = clamp(velocity, 0, maxVelocity)`，确保向下），管道和地面停止滚动，小鸟以正常重力坠落，旋转锁定 +90°（头朝下）。当 `bird.y + bird.radius >= 1.0 - ground.height` 或死亡超过 1.0s 时转入 GameOver
