...
void main() {
  ...
  test('isDigitComplete returns true when all 9 placed', () {
    board.setValue(0, 1, 3);
    board.setValue(0, 2, 4);
    // All 5s are given in the grid
    expect(board.isDigitComplete(5), true);
  });
  ...
  test('hasConflict detects column conflict', () {
    // Column 0 already has 5 at (0,0). Place 5 at (4,1) first to have a free row,
    // but actually let's use empty cell (0,1) and place value matching col.
    // Col 1 has 3,7,9,5,2,1,6,8,4 — place 7 at (0,1) which conflicts with (1,1)
    board.setValue(0, 1, 7); // 7 already at (1,1)=row1,col1 ? No, (1,1)=7 is given
    expect(board.errorCells.contains((0, 1)), true);
  });

  test('hasConflict detects box conflict', () {
    // Box(0,0) contains 5,3,_,6,7,_,1,9,8. Place 9 at (0,1) — 9 at (0,2) would conflict
    // but (0,2) is empty. Place 8 at (0,1) — 8 is at (2,2)=8 in same box
    board.setValue(0, 1, 8); // 8 already at (2,2) in same box
    expect(board.errorCells.contains((0, 1)), true);
  });
}
```
...
class _SudokuPageState extends State<SudokuPage> {
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    ...
    // Number keys
    for (int d = 1; d <= 9; d++) {
      if (key == LogicalKeyboardKey(0x30 + d) || // digit row
          key == LogicalKeyboardKey(0xffb0 + d)) { // numpad
        _onNumberTap(d);
        return KeyEventResult.handled;
      }
    }
  ...
  Widget _buildBoard() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final cellSize = (boardSize / 9).clamp(36.0, double.infinity);
      ...
      children: List.generate(9, (r) {
        return Row(
          ...
          children: List.generate(9, (c) {
            ...
            final isRelated = _selectedRow != null &&
                (r == _selectedRow ||
                    c == _selectedCol ||
